...
"""
Register module for tpDcc-dccs-mobu
"""
...
def register_class(cls_name, cls, is_unique=False, skip_store=False):
    ...
    if not skip_store:
        tpDcc.dccs.mobu.__dict__[REGISTER_ATTR].append(cls_name)
...
def cleanup():
    ...
    for cls_name in tpDcc.dccs.mobu.__dict__[REGISTER_ATTR]:
        if cls_name not in tpDcc.dccs.mobu.__dict__:
            continue
        print('Deleting: {}'.format(cls_name))
        del tpDcc.dccs.mobu.__dict__[cls_name]
    del tpDcc.dccs.mobu.__dict__[REGISTER_ATTR]
